...
def get_hollow_mask(mask: np.ndarray, dilate_factor=15):
    kernel = np.ones((dilate_factor, dilate_factor), np.uint8)
    gradient = cv.morphologyEx(mask, cv2.MORPH_GRADIENT, kernel)

    return mask

def composite_mask(input_image, mask, position, dilate_factor=15):
    # create the target mask
    target_mask = np.zeros(input_image.shape[:2], dtype=np.uint8)
    ...
    h, w = mask.shape[:2]
    print(x , y, h, w)
    # add the mask over the target mask
    breakpoint()
    # add the mask over the target mask
    target_mask[y:y+h, x:x+w] = mask
    target_mask_hollow = get_hollow_mask(target_mask, dilate_factor=dilate_factor)
    return target_mask_hollow
