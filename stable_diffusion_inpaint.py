...
def fill_img_with_sdxl(
    ...
    img_crop, mask_crop = crop_for_filling_pre(img, mask, crop_size=1024)
    ...
    img_filled = crop_for_filling_post(img, mask, np.array(img_crop_filled), crop_size=1024)
    return img_filled    
...
def fill_img_with_sd(
    ...
    img_crop, mask_crop = crop_for_filling_pre(img, mask, crop_size=512)
    img_crop_filled = pipe(
        prompt=text_prompt,
        image=Image.fromarray(img_crop),
        mask_image=Image.fromarray(mask_crop)
    ).images[0]
    img_filled = crop_for_filling_post(img, mask, np.array(img_crop_filled),  crop_size=512)
    return img_filled
